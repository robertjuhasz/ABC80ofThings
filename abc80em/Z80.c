...
#define RdZ80 RDZ80
extern byte *ROMPage[];
INLINE byte RdZ80(sword A) { return(ROMPage[A>>13][A&0x1FFF]); }
#endif
...
INLINE byte RdZ80(sword A)        { return(Page[A>>13][A&0x1FFF]); }
INLINE void WrZ80(sword A,byte V) { if(Page[A>>13]<ROM) Page[A>>13][A&0x1FFF]=V; }
#endif
...
#define FAST_RDOP
extern byte *RAM[];
INLINE byte OpZ80(sword A) { return(RAM[A>>13][A&0x1FFF]); }
#endif
...
/*************************************************************/
void IntZ80(Z80 *R,sword Vector)
{
  ...
  if((R->IFF&IFF_1)||(Vector==INT_NMI))
  {
    ...
    if(R->IFF&IFF_IM2)
    {
      ...
      Vector=(Vector&0xFF)|((sword)(R->I)<<8);
      /* Read the vector */
